...
if __name__ == '__main__':
    ...
    for train_indices, val_indices, test_indices in dataset_handler.get_indices():
        ...
        val_texts = [
            (val_parent_texts[i], val_texts[i]) if val_parent_texts[i] != val_texts[i] else val_texts[i] for i
            in range(len(val_texts))]
        test_texts = [
            (test_parent_texts[i], test_texts[i]) if test_parent_texts[i] != test_texts[i] else test_texts[i] for i
            in range(len(test_texts))]
        ...
        model = BertMultiLabel(optimizer).get_model()
        model.summary()

        tf.keras.utils.plot_model(model, to_file='bert.png', show_shapes=True, expand_nested=True,
